...
get_targets() {
    bazel query 'kind("cc_binary", //src/...)' | grep -E "$g_only"
}
...
install_curvebs() {
    ...
    do
        ...
        local project_name="${BASH_REMATCH[2]}"  # ex: chunkserver
        g_project_name=$project_name
        local project_prefix="$g_prefix/$project_name"  # ex: /usr/local/curvebs/chunkserver

        local project_bin_filename=${BASH_REMATCH[3]}  # ex: curvebs_chunkserver
